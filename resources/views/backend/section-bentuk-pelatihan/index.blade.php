...
@section('content')
    ...
    <h4 class="card-title text-white mb-0">Tabel Data Section Bentuk Pelatihan</h4>
    ...
    <tbody>
    @forelse ($sectionbentukpelatihan as $key => $d)
        <tr>
        <td>{{ $key + 1 }}</td>
        <td>{{ $d->heading }}</td>
        <td>{{ $d->top_title }}</td>
        ...
        <a href="{{ route('admin.section-bentuk-pelatihan.edit', $d->id) }}" class="btn btn-warning btn-circle">
