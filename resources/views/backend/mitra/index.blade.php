...
@section('content')
  ...
  <div class="col-12">
    ...
    <h4 class="card-title text-white mb-0">Tabel Data Mitra</h4>
    ...
    <table id="demo-foo-row-toggler" class="table table-bordered toggle-arrow-tiny text-nowrap"
      ...
      <th data-breakpoints="sm">Logo</th>
      <th data-breakpoints="xs">Status</th>
      <th data-breakpoints="all">Nama Mitra</th>
      <th class="text-center" data-breakpoints="xs">Aksi</th>
      </tr>
      </thead>
      <tbody>
      @forelse ($mitra as $key => $d)
      <tr>
      <td>{{ $key + 1 }}</td>
      <td><img src="{{ asset('storage/' . $d->logo) }}" alt="Logo Mitra" style="max-width: 100px; max-height: 100px;"></td>
      <td>
        @if ($d->status == 'aktif')
      ...
      <td class="d-flex gap-2 text-white">
        <a href="{{ route('admin.mitra.edit', $d->id) }}" class="btn btn-warning btn-circle">
        <i class="ri-edit-2-fill"></i>
        </a>
        <form action="{{ route('admin.mitra.destroy', $d->id) }}" method="POST" class="delete-form"
