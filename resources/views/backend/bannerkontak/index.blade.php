...
@section('content')
    ...
    <div class="card">
      ...
      <h4 class="card-title text-white mb-0">Tabel Data Banner Kontak</h4>
    ...
    @endif
      ...
      <thead>
        ...
        <th data-breakpoints="xs">Teks</th>
        <th>Gambar</th>
        <th class="text-center" data-breakpoints="xs">Aksi</th>
        </tr>
      </thead>
      <tbody>
        @forelse ($bannerkontak as $key => $data)
      <tr>
      <td>{{ $key + 1 }}</td>
      <td>{{ $data->text }}</td>
      <td><img src="{{ asset('storage/' . $data->image) }}" alt="Gambar Banner"
      style="max-width: 200px; max-height: 100px;"></td>

      <td class="d-flex gap-2 text-white">
      <a href="{{ route('admin.bannerkontak.edit', $data->id) }}" class="btn btn-warning btn-circle">
      <i class="ri-edit-2-fill"></i>
      </a>
      <form action="{{ route('admin.bannerkontak.destroy', $data->id) }}" method="POST" class="delete-form"
