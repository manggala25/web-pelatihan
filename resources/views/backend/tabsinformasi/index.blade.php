...
@section('content')
    ...
    <h4 class="card-title text-white mb-0">Tabel Data 3 Tabs Informasi</h4>
    ...
    <th data-breakpoints="xs">Judul Informasi</th>
    <th data-breakpoints="xs">Isi Konten</th>
    <th class="text-center" data-breakpoints="xs">Aksi</th>
    </tr>
    </thead>
    <tbody>
    @forelse ($tabsinformasi as $key => $data)
    <tr>
    <td>{{ $key + 1 }}</td>
    <td>{{ $data->nama_tabs }}</td>
    <td>{!! Str::limit(strip_tags($data->content), 30) !!}</td>

    <td class="d-flex gap-2 text-white">
    <a href="{{ route('admin.tabsinformasi.show', $data->id) }}" class="btn btn-info btn-circle">
    <i class="ri-eye-line"></i>
    </a>
    <a href="{{ route('admin.tabsinformasi.edit', $data->id) }}" class="btn btn-warning btn-circle">
    <i class="ri-edit-2-fill"></i>
    </a>
    <form action="{{ route('admin.tabsinformasi.destroy', $data->id) }}" method="POST" class="delete-form"
