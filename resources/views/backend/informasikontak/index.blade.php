        ...
        <div class="col-12">
            <div class="card">
                <div
                    ...
                    <h4 class="card-title text-white mb-0">Tabel Data Informasi Kontak</h4>
                    <input id="searchInput" type="text" class="form-control w-25" placeholder="Cari nama kontak..." />
                </div>
                <div class="card-body">
                    @if (session('success'))
                        ...
                        <table id="demo-foo-row-toggler" class="table table-bordered toggle-arrow-tiny text-nowrap"
                            ...
                            <thead>
                                <tr>
                                    ...
                                    <th data-breakpoints="xs sm">Nama Kontak</th>
                                    <th data-breakpoints="xs">Link</th>
                                    <th class="text-center" data-breakpoints="xs">Aksi</th>
                                </tr>
                            </thead>
                            <tbody>
                                @forelse ($informasi_kontak as $key => $d)
                                    <tr>
                                        <td>{{ $key + 1 }}</td>
                                        <td>{{ Str::limit($d->nama_kontak) }}</td>
                                        <td>{{ Str::limit($d->link, 50) }}</td>
                                        <td class="d-flex gap-2 text-white">
                                            <a href="{{ route('admin.informasikontak.show', $d->id) }}" class="btn btn-info btn-circle">
                                                <i class="ri-eye-line"></i>
                                            </a>
                                            <a href="{{ route('admin.informasikontak.edit', $d->id) }}"
                                                class="btn btn-warning btn-circle">
                                                <i class="ri-edit-2-fill"></i>
                                            </a>
                                            <form action="{{ route('admin.informasikontak.destroy', $d->id) }}"
                                                method="POST"
