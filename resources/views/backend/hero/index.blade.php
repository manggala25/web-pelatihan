...
@section('content')
    ...
    <h4 class="card-title text-white mb-0">Tabel Data Hero Section</h4>
    ...
    <tbody>
    @forelse ($hero as $key => $hero)
        <tr>
        <td>{{ $key + 1 }}</td>
        <td>{{ Str::limit($hero->heading, 50) }}</td>
        <td>{{ $hero->top_title }}</td>
        <td class="d-flex gap-2 text-white">
        <a href="{{ route('admin.hero.show', $hero->id) }}" class="btn btn-info btn-circle">
        <i class="ri-eye-line"></i>
        </a>
        <a href="{{ route('admin.hero.edit', $hero->id) }}" class="btn btn-warning btn-circle">
