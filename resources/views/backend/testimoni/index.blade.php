...
@section('content')
    ...
    <h4 class="card-title text-white mb-0">Tabel Data Testimoni</h4>
    ...
    <th data-breakpoints="xs sm">Nama PIC</th>
    <th data-breakpoints="xs sm">Jabatan/ Okupasi</th>
    <th data-breakpoints="all">Nama Lembaga</th>
    <th class="text-center" data-breakpoints="xs">Aksi</th>
    </tr>
    </thead>
    <tbody>
    @forelse ($testimoni as $key => $d)
        <tr>
        <td>{{ $key + 1 }}</td>
        <td>{{ Str::limit($d->nama_pic, 50) }}</td>
        <td>{{ Str::limit($d->okupasi, 50) }}</td>
        <td>{{ $d->nama_lembaga }}</td>
        <td class="d-flex gap-2 text-white">
        <a href="{{ route('admin.testimoni.show', $d->id) }}" class="btn btn-info btn-circle">
        <i class="ri-eye-line"></i>
        </a>
        <a href="{{ route('admin.testimoni.edit', $d->id) }}" class="btn btn-warning btn-circle">
        <i class="ri-edit-2-fill"></i>
        </a>
        <form action="{{ route('admin.testimoni.destroy', $d->id) }}" method="POST"
