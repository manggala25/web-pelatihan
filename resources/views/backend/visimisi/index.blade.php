...
@section('content')
    ...
    <h4 class="card-title text-white mb-0">Tabel Data Visi & Misi</h4>
    ...
    <tbody>
    @forelse ($visimisi as $key => $visimisi)
        <tr>
        <td>{{ $key + 1 }}</td>
        <td>{{ Str::limit($visimisi->heading, 50) }}</td>
        <td>{{ $visimisi->title_top }}</td>
        <td class="d-flex gap-2 text-white">
        <a href="{{ route('admin.visimisi.show', $visimisi->id) }}" class="btn btn-info btn-circle">
        <i class="ri-eye-line"></i>
        </a>
        <a href="{{ route('admin.visimisi.edit', $visimisi->id) }}" class="btn btn-warning btn-circle">
