...
@section('content')
    ...
    <h4 class="card-title text-white mb-0">Tabel Data Target Pelatihan</h4>
    ...
    <tbody>
    @forelse ($targetpelatihan as $key => $d)
        <tr>
        <td>{{ $key + 1 }}</td>
        <td>{{ Str::limit($d->heading, 50) }}</td>
        <td>{{ $d->top_title }}</td>
        <td class="d-flex gap-2 text-white">
        <a href="{{ route('admin.target-pelatihan.show', $d->id) }}" class="btn btn-info btn-circle">
        <i class="ri-eye-line"></i>
        </a>
        <a href="{{ route('admin.target-pelatihan.edit', $d->id) }}" class="btn btn-warning btn-circle">
